import random
from Corak.dicio import salas
from Corak.dicio import U, R, L, D
class keep():
    ...
    for andar in range(len(level)):
        for sala in range(len(level[andar])):
            try:
                ...
                level[andar][sala] = getLinkExc(level, andar, sala, link)
    try:
        if "hall" not in level[keeper[0].at[1]][keeper[0].at[0]] or\
           "end." not in level[keeper[0].at[1] + D][keeper[0].at[0]] or\
           "strt" not in level[keeper[0].at[1] + U][keeper[0].at[0]] or\
           "key1" not in level[keeper[0].at[1]][keeper[0].at[0] + R] or\
           "key2" not in level[keeper[0].at[1]][keeper[0].at[0] + L]:
            return [0, 0, 0, 0]
        else: return level
    except: return [0, 0, 0, 0]
