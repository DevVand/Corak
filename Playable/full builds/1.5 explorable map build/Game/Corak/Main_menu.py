...
while Active:
    ...
    if menu_area:
                    ...
                    if clickset:
                        if not opt_menu:
                            ...
                            if "options" in button.type:
                                ...
                                opt_menu = True
                            if "credits" in button.type: credits_menu = True
                            if "exit" in button.type:
                                out = 0
                                temp2 = True
                        else:
                            if "small" in button.type:
                                mixerOn[3] = True
                                mixerOn[4][0] = True
                                ...
                                mixerOn[5][3] = True
                                out = 0
                            elif "medium" in button.type: pointer.setFrame([0, pointer.frame[1] + 1])
                            elif "large" in button.type: pass
                            elif "back" in button.type: opt_menu = False
                            elif "pointer1" in button.type: pointer.setFrame([0, 0])
                            elif "pointer2" in button.type: pointer.setFrame([0, 1])
                            elif "pointer3" in button.type: pointer.setFrame([0, 2])
                            elif "pointer4" in button.type: pointer.setFrame([0, 3])
                            elif "vol0" in button.type:
                                button.setFrame([0, 1])
                                if Evol > 0:
                                    Evol -= 1
                                    select.set_volume((0.9 / 10) * Evol)
                                    clicks.set_volume((0.8 / 10) * Evol)
                                    click2.set_volume((0.7 / 10) * Evol)
                            elif "vol1" in button.type:
                                button.setFrame([0, 1])
                                if Evol < 10:
                                    Evol += 1
                                    select.set_volume((0.9 / 10) * Evol)
        ...
        for button in menu[i]: screen.image.blit(button.image, button.rect)
        if opt_menu:
