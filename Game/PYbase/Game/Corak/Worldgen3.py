import random
from Game.Corak.dicio import salas, U, R, L, D, mapSize, rules
class keep():
...
def insert(level, sala):
    ...
    if "U" in sala: Umod *= 2
    if "D" in sala: Dmod *= 2
    if "_hall" in sala: Umod = Dmod = Rmod = Lmod = 2
    if rules[0] and "_hall" in sala: Umod = Dmod = Rmod = Lmod = rules[3]
    while True:
        if counter == 200: return [0, 0, 0, 0]
        counter += 1
        y = random.randint(0 + Umod, mapSize[1] - 1 - Dmod)
        x = random.randint(0 + Lmod, mapSize[0] - 1 - Rmod)
        if "_hall" not in sala:
            if "X" not in (level[y][x] or level[y+1][x] or level[y][x+1] or level[y-1][x] or level[y][x-1]):
                ...
                if rules[0]:
                    if "_key1" in sala:
                        excDistance = rules[2]
                        maxDistance = (mapSize[0] / 2) + (mapSize[1] / 2)
...
def fixer(level):
    ...
    try:
        for i in range(len(keeper)-1):
            alltrue = False
            for a in rules[1]:
                if a in level[keeper[i].at[1]][keeper[i].at[0]]:
                    alltrue = True

            if not alltrue: return [0, 0, 0, 0]
        else: return level
    except: return [0, 0, 0, 0]
        ...
        for sala in open("../Salas/tokeeplist.txt", 'r'):
            ...
            if "x" in sala:
                ...
                if alltrue:
                    ...
                    if "_hall" not in temp4:

                        level = pathFinder(level, temp4, at[0], at[1])
